...
export interface ColocardConfig {
    mongodb: {
        database: string,
        host: string | string[],
...
export function isValidColocardLogLevel(level: string): boolean {
    const values = _.values(ColocardLogLevel);
    return _.contains(level, values);
}

export default class Colocard {
    public readonly configuration: ColocardConfig;
    private logger: pino.BaseLogger;
    private server: Server;
    constructor(config: ColocardConfig) {
        this.configuration = config;
        if (isValidColocardLogLevel(this.configuration.server.logLevel)) {
            this.logger = pino({
    ...
    public async start() {
        try {
            this.logger.info({ config: this.configuration }, "starting colocard instance");
            await this.launch();
        } catch (err) {
            this.logger.fatal({ error: err }, "failed to start colocard instance");
            throw err;
        }
    }
    ...
    private async launch() {
        ...
        this.logger.info("colocard instance running");
    }

    public async stop() {
        try {
            this.logger.info("shutting down colocard instance");
            this.halt();
        } catch (err) {
            this.logger.fatal({ error: err }, "failed to gracefully shutdown colocard instance");
            throw err;
        }
    }
